...
while True:
    ...
    faces = face_cascade.detectMultiScale(gray, 1.3, 5)  #1.3 - scalefactorr 
    bodies = body_cascade.detectMultiScale(gray, 1.3, 5) # 5 minNeighbours  high values less detection
    ...
    if  len(faces) +  len(bodies) + len(upper) > 0  or sys.getsizeof(contours) >240 :
        ...
        if detection:
            ...
            detection = True
            current_time = datetime.now().strftime("%d-%B-%Y___%H:%S %p ")
            p = os.path.sep.join(['shots', "video_{}.mp4".format(str(exact_time).replace(":",''))])
            out = cv2.VideoWriter(p, fourcc, 20, frame_size)
    ...
    if count == 1:
        current_time = datetime.now().strftime("%d-%m-%Y_____%H:%S %p ")
    ...
    for (x, y, width, height) in faces:
        ...
        exact_time=datetime.now().strftime('%d-%B-%Y___%H:%S %p ')

        p = os.path.sep.join(['shots', "face detected_{}.png".format(str(exact_time).replace(":",''))])
        cv2.imwrite(p, img)
